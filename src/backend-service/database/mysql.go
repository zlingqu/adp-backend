...
type Config struct {
	mysql.Config
	MaxIdleConnections int // 连接池连接数 = mysql最大连接数/2
	MaxOpenConnections int // 最大打开连接 = mysql最大连接数
...
func Init(config Config) {
	...
	if err != nil {
		...
		SqlDB.SetMaxIdleConns(config.MaxIdleConnections) // 连接池连接数 = mysql最大连接数/2
		SqlDB.SetMaxOpenConns(config.MaxOpenConnections) // 最大打开连接 = mysql最大连接数
	...
	// 初始化其他连接
	for k, v := range config.Connections {
		...
		tempSql.SetMaxIdleConns(v.MaxIdleConnections) // 连接池连接数 = mysql最大连接数/2
		tempSql.SetMaxOpenConns(v.MaxOpenConnections) // 最大打开连接 = mysql最大连接数
		sqlDBmap[k] = tempSql
	}
}
