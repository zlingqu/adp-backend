...
func GetPodsStatus(c *gin.Context) {
	...
	//pods status
	pt := NewPodsStatus()
	pt.SetKubernetesConfigFilePath(env).GetKubernetesClient()
...
func (p *PodsStatus) SetKubernetesConfigFilePath(env string) *PodsStatus {
	// get config file
	p.KubernetesConfigFile = conf.GetConfFile(env)
	return p
}
